    ...
    subscription: new graphql_2.GraphQLObjectType({
        ...
        fields: {
            ...
            userFiltered: {
                ...
                subscribe: (0, graphql_subscriptions_1.withFilter)(function () { return testPubsub.asyncIterator('userFiltered'); }, function (user, args) {
...
var httpServer = (0, http_1.createServer)(notFoundRequestListener);
httpServer.listen(TEST_PORT);
new server_1.SubscriptionServer(options, { server: httpServer });
var httpServerWithKA = (0, http_1.createServer)(notFoundRequestListener);
...
var httpServerWithEvents = (0, http_1.createServer)(notFoundRequestListener);
...
var httpServerWithOnConnectError = (0, http_1.createServer)(notFoundRequestListener);
...
var httpServerWithDelay = (0, http_1.createServer)(notFoundRequestListener);
...
var httpServerRaw = (0, http_1.createServer)(notFoundRequestListener);
...
describe('Client', function () {
    ...
    it('should send GQL_CONNECTION_INIT message when creating the connection', function (done) {
        wsServer.on('connection', function (connection) {
            connection.on('message', function (message) {
                ...
                (0, chai_1.expect)(parsedMessage.type).to.equals(message_types_1.default.GQL_CONNECTION_INIT);
                done();
            });
        });
        new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"));
    });
    it('should subscribe once after reconnect', function (done) {
        ...
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), {
            reconnect: true,
        ...
        setTimeout(function () {
            (0, chai_1.expect)(subscriptionsCount).to.be.equal(1);
            done();
        }, 1500);
    });
    it('should send GQL_CONNECTION_INIT message first, then the GQL_START message', function (done) {
        ...
        wsServer.on('connection', function (connection) {
            connection.on('message', function (message) {
                ...
                if (parsedMessage.type === message_types_1.default.GQL_START) {
                    (0, chai_1.expect)(initReceived).to.be.true;
                    if (sub) {
                        sub.unsubscribe();
                        done();
                    }
    ...
    it('should emit connect event for client side when socket is open', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
    ...
    it('should emit connected event for client side when socket closed', function (done) {
        ...
        var unregister = client.onConnected(function () {
            ...
            unregister();
            (0, chai_1.expect)(onConnectingSpy.called).to.equal(true);
            done();
        });
    });
    it('should emit connecting event for client side when socket closed', function (done) {
        var subscriptionsClient = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        ...
        var unregisterOnConnecting = subscriptionsClient.onConnecting(function () {
            ...
            (0, chai_1.expect)(onConnectedSpy.called).to.equal(false);
            done();
        });
    });
    ...
    it('should emit reconnected event for client side when socket closed', function (done) {
        ...
        var unregister = client.onReconnected(function () {
            ...
            unregister();
            (0, chai_1.expect)(onReconnectingSpy.called).to.equal(true);
            done();
        });
    });
    it('should emit reconnecting event for client side when socket closed', function (done) {
        ...
        var unregisterOnReconnecting = subscriptionsClient.onReconnecting(function () {
            ...
            (0, chai_1.expect)(onReconnectedSpy.called).to.equal(false);
            done();
        });
    });
    it('should throw an exception when query is not provided', function (done) {
        ...
        client.request({
            ...
            next: function () { return (0, chai_1.assert)(false); },
            error: function (error) {
                client.close();
                (0, chai_1.expect)(error.message).to.be.equal('Must provide a query.');
                done();
            },
        });
    });
    ...
    it('should allow both data and errors on GQL_DATA', function (done) {
        ...
        client.request({
            ...
            next: function (result) {
                (0, chai_1.expect)(result.data).to.have.property('some');
                (0, chai_1.expect)(result.errors).to.be.lengthOf(1);
                done();
            },
        });
    });
    it('should send connectionParams along with init message', function (done) {
        ...
        wsServer.on('connection', function (connection) {
            connection.on('message', function (message) {
                var parsedMessage = JSON.parse(message);
                (0, chai_1.expect)(JSON.stringify(parsedMessage.payload)).to.equal(JSON.stringify(connectionParams));
                done();
            });
        });
        new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), {
    ...
    it('should catch errors in connectionParams which came from a promise', function (done) {
        ...
        wsServer.on('connection', function (connection) {
            connection.on('message', function (message) {
                var parsedMessage = JSON.parse(message);
                (0, chai_1.expect)(parsedMessage.payload).to.equal(error);
                done();
            });
        });
        new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), {
    ...
    it('should override OperationOptions with middleware', function (done) {
        var client3 = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        ...
        client3.request({
            ...
            next: function (result) {
                try {
                    ...
                    if (result.errors) {
                        (0, chai_1.assert)(false, 'got error during subscription creation');
                    }
                    if (result.data) {
    ...
    it('should handle correctly GQL_CONNECTION_ERROR message', function (done) {
        ...
        new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), {
            connectionCallback: function (error) {
                (0, chai_1.expect)(error.message).to.equals('test error');
                done();
            },
        });
    });
        ...
        wsServer.on('connection', function (connection) {
            connection.on('message', function (message) {
                connection.send(JSON.stringify({
                    ...
                    connection.close();
                    setTimeout(function () {
                        (0, chai_1.expect)(client.status).to.equals(WebSocket.CLOSED);
                        done();
                    }, 500);
                });
            });
        });
        client = new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"));
    });
    it('should handle correctly GQL_CONNECTION_ACK message', function (done) {
        ...
        new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), {
            connectionCallback: function (error) {
                (0, chai_1.expect)(error).to.equals(undefined);
                done();
            },
        });
    });
    it('should handle correctly GQL_CONNECTION_ACK payload for onConnected', function (done) {
        var server = (0, http_1.createServer)(notFoundRequestListener);
        ...
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(ACK_ONCONNECTED_TEST_PORT, "/"));
        client.onConnected(function (payload) {
            (0, chai_1.expect)(payload.appVersion).to.equal('1.0.0');
            done();
        });
    });
    it('should handle correctly GQL_CONNECTION_ACK payload for onReconnected', function (done) {
        ...
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(ACK_ONRECONNECTED_TEST_PORT, "/"), { reconnect: true });
        ...
        return new Promise(function (resolve, reject) {
            var sub = client.request({
                ...
                next: function (result) {
                    try {
                        sub.unsubscribe();
                        (0, chai_1.expect)(Object.keys(client.operations).length).to.equals(0);
                        resolve(undefined);
                    }
                    catch (e) {
                        reject(e);
    ...
    it('queues messages while websocket is still connecting', function (done) {
        ...
        client.onConnecting(function () {
            (0, chai_1.expect)(client.unsentMessagesQueue.length).to.equals(1);
            sub.unsubscribe();
            setTimeout(function () {
                (0, chai_1.expect)(client.unsentMessagesQueue.length).to.equals(0);
                done();
            }, 100);
        });
    });
    ...
    it('should call error handler when graphql query is not valid', function (done) {
        ...
        setTimeout(function () {
            client.request({
                ...
                next: function (result) {
                    if (result.errors.length) {
                        (0, chai_1.expect)(result.errors[0].message).to.equals('Cannot query field "invalid" on type "Subscription".');
                        done();
                    }
                    else {
                        (0, chai_1.assert)(false);
    ...
    function testBadServer(payload, errorMessage, done) {
        ...
        client.request({
            ...
            next: function () { return (0, chai_1.assert)(false); },
            error: function (error) {
                (0, chai_1.expect)(error.message).to.equals(errorMessage);
                done();
            },
        });
    }
        ...
        wsServer.on('connection', function (connection) {
            connection.on('message', function () {
                if (!isDone) {
                    isDone = true;
                    try {
                        (0, chai_1.expect)(client.client).to.not.be.null;
                        sub.unsubscribe();
                        done();
                    }
                    catch (e) {
            ...
            connection.on('message', function (message) {
                if (!isDone) {
                    ...
                    try {
                        ...
                        (0, chai_1.expect)(parsedMessage.payload).to.eql({
                            foo: 'bar',
                        });
                        done();
                    }
    ...
    it('should reconnect to the server', function (done) {
        ...
        wsServer.on('connection', function (connection) {
            ...
            if (connections === 1) {
                ...
                (0, chai_1.expect)(client.client).to.not.be.equal(originalClient);
                done();
            }
        });
        client = new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), { reconnect: true });
    ...
    it('should emit event when an websocket error occurs', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(ERROR_TEST_PORT, "/"));
        client.request({
            query: "subscription useInfo{\n        invalid\n      }",
            variables: {},
        }).subscribe({
        ...
        client.onError(function (err) {
            (0, chai_1.expect)(err.message).to.be.equal("connect ECONNREFUSED 127.0.0.1:".concat(ERROR_TEST_PORT));
            done();
        });
    });
    it('should stop trying to reconnect to the server', function (done) {
        ...
        var errorCount = 0;
        var subscriptionsClient = new client_1.SubscriptionClient("ws://localhost:".concat(RAW_TEST_PORT, "/"), {
            timeout: 500,
            reconnect: true,
            reconnectionAttempts: 2,
        });
        subscriptionsClient.onError(function (error) {
            (0, chai_1.expect)(error.message).to.contain('A message was not sent');
            errorCount += 1;
        });
        var connectSpy = sinon.spy(subscriptionsClient, 'connect');
        setTimeout(function () {
            (0, chai_1.expect)(connectSpy.callCount).to.be.equal(2);
            (0, chai_1.expect)(errorCount).to.be.equal(1);
            done();
        }, 1500);
    });
    ...
    it('should keep trying to reconnect if receives the ack from the server', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.expect)(connections).to.be.greaterThan(3);
            (0, chai_1.expect)(connectSpy.callCount).to.be.greaterThan(2);
            wsServer.close();
            done();
        }, 1900);
    });
    it('should take care of received keep alive', function (done) {
        ...
        var subscriptionsClient = new client_1.SubscriptionClient("ws://localhost:".concat(KEEP_ALIVE_TEST_PORT, "/"), { timeout: 600 });
        ...
        setTimeout(function () {
            (0, chai_1.expect)(wasKAReceived).to.equal(true);
            (0, chai_1.expect)(subscriptionsClient.status).to.equal(WebSocket.CLOSED);
            done();
        }, 1200);
    });
    it('should correctly clear timeout if receives ka too early', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.expect)(checkConnectionSpy.callCount).to.be.equal(receivedKeepAlive);
            (0, chai_1.expect)(subscriptionsClient.status).to.be.equal(subscriptionsClient.client.OPEN);
            done();
        }, 1300);
    });
    ...
    it('should not send GQL_STOP on unsubscribe after operation received a GQL_COMPLETE', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        var sendMessageSpy = sinon.spy(client, 'sendMessage');
        client.onConnected(function () {
            var subscription = client.request({
            ...
            setTimeout(function () {
                (0, chai_1.expect)(sendMessageSpy.calledWith(undefined, 'connection_init', sinon.match.any)).to.be.true;
                (0, chai_1.expect)(sendMessageSpy.calledWith('1', 'start', sinon.match.any)).to.be.true;
                (0, chai_1.expect)(sendMessageSpy.calledWith('1', 'stop', sinon.match.any)).to.be.false;
                done();
            }, 1000);
        });
    });
    it('should delete operation when receive a GQL_COMPLETE', function (done) {
        ...
        (0, chai_1.expect)(subscriptionsClient.operations).to.have.property('1');
        originalOnMessage(dataToSend);
        (0, chai_1.expect)(subscriptionsClient.operations).to.not.have.property('1');
        done();
    });
    it('should force close the connection without tryReconnect', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.expect)(receivedConnecitonTerminate).to.be.equal(true);
            (0, chai_1.expect)(tryReconnectSpy.callCount).to.be.equal(0);
            (0, chai_1.expect)(subscriptionsClient.status).to.be.equal(WebSocket.CLOSED);
            done();
        }, 500);
    });
    it('should close the connection without sent connection terminate and reconnect', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.expect)(tryReconnectSpy.callCount).to.be.equal(1);
            (0, chai_1.expect)(subscriptionsClient.status).to.be.equal(WebSocket.OPEN);
            (0, chai_1.expect)(receivedConnecitonTerminate).to.be.equal(false);
            done();
        }, 500);
    });
    it('should close the connection after inactivityTimeout and zero active subscriptions', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.expect)(Object.keys(subscriptionsClient.operations).length).to.be.equal(1);
            sub.unsubscribe();
            setTimeout(function () {
                (0, chai_1.expect)(Object.keys(subscriptionsClient.operations).length).to.be.equal(0);
                setTimeout(function () {
                    (0, chai_1.expect)(subscriptionsClient.status).to.be.equal(WebSocket.CLOSED);
                    done();
                }, 101);
    ...
    it('should allow passing custom WebSocket protocols', function () {
        ...
        for (var _i = 0, testCases_1 = testCases; _i < testCases_1.length; _i++) {
            ...
            new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT), {}, mockWebSocket, testCase);
            (0, chai_1.expect)(mockWebSocket.calledOnce).to.be.true;
            (0, chai_1.expect)(mockWebSocket.firstCall.args[1]).to.equal(testCase);
        }
    });
});
describe('Server', function () {
    ...
    it('should throw an exception when schema is not provided', function (done) {
        server = (0, http_1.createServer)(notFoundRequestListener);
        ...
        var errorMessage;
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(SERVER_EXECUTOR_TESTS_PORT, "/"));
        client.onConnected(function () {
            client.request({
                ...
                next: function (res) {
                    (0, chai_1.assert)(false, 'expected error to be thrown');
                },
                error: function (err) {
                    errorMessage = err.message;
                    (0, chai_1.expect)(errorMessage).to.contain('Missing schema information');
                    done();
                },
                complete: function () {
                    (0, chai_1.assert)(false, 'expected error to be thrown');
    ...
    it('should use schema provided in onOperation', function (done) {
        ...
        client.onConnected(function () {
            client.request({
                ...
                next: function (res) {
                    if (res.errors) {
                        (0, chai_1.assert)(false, 'unexpected error from request');
                    }
                    (0, chai_1.expect)(res.data).to.deep.equal({ testString: 'value' });
                    msgCnt++;
                },
                error: function (err) {
                    (0, chai_1.assert)(false, 'unexpected error from request');
                ...
                complete: function () {
                    (0, chai_1.expect)(msgCnt).to.equals(1);
                    done();
                },
            });
        });
    ...
    it('should have request interface (apollo client 2.0)', function (done) {
        ...
        client.onConnected(function () {
            ...
            client.request({
                ...
                next: function (res) {
                    (0, chai_1.expect)(hasValue).to.equal(false);
                    (0, chai_1.expect)(res).to.deep.equal({ data: { testString: 'value' } });
                    hasValue = true;
                },
                error: function (err) {
    ...
    it('should handle socket error and close the connection on error', function (done) {
        var spy = sinon.spy();
        var httpServerForError = (0, http_1.createServer)(notFoundRequestListener);
        ...
        new server_1.SubscriptionServer({
            ...
            onConnect: function (payload, socket) {
                setTimeout(function () {
                    ...
                    setTimeout(function () {
                        (0, chai_1.assert)(spy.calledOnce);
    ...
    it('should trigger onConnect when client connects and validated', function (done) {
        new client_1.SubscriptionClient("ws://localhost:".concat(EVENTS_TEST_PORT, "/"));
        setTimeout(function () {
            (0, chai_1.assert)(eventsOptions.onConnect.calledOnce);
            done();
        }, 200);
    });
    it('should trigger onConnect with the correct connectionParams', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.assert)(eventsOptions.onConnect.calledOnce);
            (0, chai_1.expect)(JSON.stringify(eventsOptions.onConnect.getCall(0).args[0])).to.equal(JSON.stringify(connectionParams));
            done();
        }, 200);
    });
    it('should trigger onConnect with the request available in ConnectionContext', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.assert)(eventsOptions.onConnect.calledOnce);
            (0, chai_1.expect)(eventsOptions.onConnect.getCall(0).args[2].request).to.be.an.instanceof(http_1.IncomingMessage);
            done();
        }, 200);
    });
    it('should trigger onConnect and return GQL_CONNECTION_ERROR with error', function (done) {
        ...
        var subscriptionsClient = new client_1.SubscriptionClient("ws://localhost:".concat(ONCONNECT_ERROR_TEST_PORT, "/"), {
            connectionCallback: connectionCallbackSpy,
        });
        setTimeout(function () {
            (0, chai_1.expect)(connectionCallbackSpy.calledOnce).to.be.true;
            (0, chai_1.expect)(connectionCallbackSpy.getCall(0).args[0]).to.eql({ message: 'Error' });
            subscriptionsClient.close();
            done();
        }, 200);
    });
    it('should trigger onConnect and return INIT_FAIL with error', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.expect)(connectionCallbackSpy.calledOnce).to.be.true;
            (0, chai_1.expect)(connectionCallbackSpy.getCall(0).args[0]).to.eql({ error: 'Error' });
            subscriptionsClient.close();
            done();
        }, 200);
    });
    it('should trigger onDisconnect when client disconnects', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(EVENTS_TEST_PORT, "/"));
        setTimeout(function () {
            client.client.close();
        }, 100);
        setTimeout(function () {
            (0, chai_1.assert)(eventsOptions.onDisconnect.calledOnce);
            done();
        }, 200);
    });
    it('should trigger onDisconnect with ConnectionContext as second argument', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.assert)(eventsOptions.onDisconnect.calledOnce);
            (0, chai_1.expect)(eventsOptions.onConnect.getCall(0).args[1]).to.not.be.undefined;
            done();
        }, 200);
    });
    ...
    it('should trigger onOperation when client subscribes', function (done) {
        ...
        setTimeout(function () {
            (0, chai_1.assert)(eventsOptions.onOperation.calledOnce);
            done();
        }, 200);
    });
        ...
        var sub = client.request({
            ...
            next: function (result) {
                ...
                if (result.data) {
                    sub.unsubscribe();
                    setTimeout(function () {
                        (0, chai_1.assert)(eventsOptions.onOperationComplete.calledOnce);
                        done();
                    }, 200);
                }
            },
    ...
    it('should send correct results to multiple clients with subscriptions', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        var client1 = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        var numResults = 0;
        setTimeout(function () {
            client.request({
                query: "subscription useInfo($id: String) {\n          user(id: $id) {\n            id\n            name\n          }\n        }",
        ...
        var client11 = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        var numResults1 = 0;
        setTimeout(function () {
            client11.request({
                query: "subscription useInfo($id: String) {\n          user(id: $id) {\n            id\n            name\n          }\n        }",
        ...
        setTimeout(function () {
            client.unsubscribeAll();
            (0, chai_1.expect)(numResults).to.equals(1);
            client1.unsubscribeAll();
            (0, chai_1.expect)(numResults1).to.equals(1);
            done();
        }, 400);
    });
    ...
    it('should set up the proper filters when subscribing', function (done) {
        var numTriggers = 0;
        var client3 = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        var client4 = new client_1.SubscriptionClient("ws://localhost:".concat(TEST_PORT, "/"));
        setTimeout(function () {
            client3.request({
    ...
    it('passes through webSocketRequest to onOperation', function (done) {
        ...
        setTimeout(function () {
            client.close();
            (0, chai_1.assert)(onOperationSpy.calledOnce);
            (0, chai_1.expect)(onOperationSpy.getCall(0).args[2]).to.not.be.undefined;
            done();
        }, 100);
    });
    ...
    it('rejects a client that does not specify a supported protocol', function (done) {
        var client = new WebSocket("ws://localhost:".concat(TEST_PORT, "/"));
        client.on('close', function (code) {
            (0, chai_1.expect)(code).to.be.eq(1002);
            done();
        });
    });
    it('rejects unparsable message', function (done) {
        var client = new WebSocket("ws://localhost:".concat(TEST_PORT, "/"), protocol_1.GRAPHQL_SUBSCRIPTIONS);
    ...
    it('sends a keep alive signal in the socket', function (done) {
        var client = new WebSocket("ws://localhost:".concat(KEEP_ALIVE_TEST_PORT, "/"), protocol_1.GRAPHQL_SUBSCRIPTIONS);
        var yieldCount = 0;
        client.onmessage = function (message) {
...
describe('Client<->Server Flow', function () {
    ...
    it('should reconnect after inactivityTimeout closing the connection and then resubscribing', function (done) {
        ...
        client.onConnected(function () { return __awaiter(void 0, void 0, void 0, function () {
                        ...
                        setTimeout(function () {
                            ...
                            setTimeout(function () {
                                var sub2 = client.request({
                                    ...
                                    next: function (res) {
                                        (0, chai_1.expect)(sub2).not.to.eq(null);
                                        (0, chai_1.expect)(res.errors).to.equals(undefined);
                                        (0, chai_1.expect)(res.data.testString).to.eq('value');
                                        sub2.unsubscribe();
                                        done();
                                    },
                                });
                ...
                if (isFirstTime) {
                    ...
                    setTimeout(function () {
                        ...
                        var sub = client.request({
                            ...
                            next: function (res) {
                                (0, chai_1.expect)(sub).not.to.eq(null);
                                (0, chai_1.expect)(res.errors).to.equals(undefined);
                                (0, chai_1.expect)(res.data.testString).to.eq('value');
                                sub.unsubscribe();
                                done();
                ...
                if (isFirstTime) {
                    ...
                    setTimeout(function () {
                        ...
                        var sub = client.request({
                            ...
                            next: function (res) {
                                (0, chai_1.expect)(sub).not.to.eq(null);
                                (0, chai_1.expect)(res.data).to.eq(undefined);
                                (0, chai_1.expect)(res.errors[0].message).to.eq('Cannot query field "invalid" on type "Query".');
                                sub.unsubscribe();
                                done();
                            },
                        });
    ...
    it('should close iteration over AsyncIterator when client unsubscribes', function () { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 3:
                    _a.sent();
                    (0, chai_1.expect)(client1.spy.callCount).to.eq(1);
                    (0, chai_1.expect)(client2.spy.callCount).to.eq(1);
                    (0, chai_1.expect)(subscriptionAsyncIteratorSpy.callCount).to.eq(2);
                    (0, chai_1.expect)(resolveAsyncIteratorSpy.callCount).to.eq(2);
                    subscriptionAsyncIteratorSpy.resetHistory();
                    resolveAsyncIteratorSpy.resetHistory();
                ...
                case 5:
                    _a.sent();
                    (0, chai_1.expect)(client1.spy.callCount).to.eq(0);
                    (0, chai_1.expect)(client2.spy.callCount).to.eq(1);
                    (0, chai_1.expect)(resolveAsyncIteratorSpy.callCount).to.eq(1);
                    (0, chai_1.expect)(subscriptionAsyncIteratorSpy.callCount).to.eq(0);
                    client2.unsubscribe();
                    return [2];
            }
        });
    }); });
    it('should close iteration over AsyncIterator when client disconnects', function () { return __awaiter(void 0, void 0, void 0, function () {
        var createClientAndSubscribe, client1, client2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
    ...
    it('should handle correctly multiple subscriptions one after each other', function (done) {
        ...
        client.onConnected(function () {
            client.request({
                ...
                next: function (res) {
                    (0, chai_1.assert)(res.errors === undefined, 'unexpected error from query');
                    (0, chai_1.expect)(res.data).to.deep.equal({ testString: 'value' });
                    var firstSub = client.request({
                        query: "subscription {\n              user(id: \"3\") {\n                id\n                name\n              }\n            }",
                    }).subscribe({
                        next: function (sRes) {
                            (0, chai_1.assert)(sRes.errors === undefined, 'unexpected error from 1st subscription');
                            (0, chai_1.assert)(sRes.data, 'unexpected null from 1st subscription result');
                            (0, chai_1.expect)(Object.keys(client['operations']).length).to.eq(1);
                            (0, chai_1.expect)(sRes.data.user).to.include({ id: '3' });
                            ...
                            setTimeout(function () {
                                client.request({
                                    ...
                                    next: function (s2Res) {
                                        (0, chai_1.assert)(s2Res.errors === undefined, 'unexpected error from 2nd subscription');
                                        (0, chai_1.assert)(s2Res.data !== null, 'unexpected null from 2nd subscription result');
                                        (0, chai_1.expect)(s2Res.data.user).to.include({ id: '1' });
                                        (0, chai_1.expect)(Object.keys(client['operations']).length).to.eq(1);
                                        (0, chai_1.expect)(firstSubscriptionSpy.callCount).to.eq(1);
                                        done();
                                    },
                                });
                            }, 10);
    ...
    it('works with custom WebSocket implementation', function (done) {
        ...
        var customServer = new MockServer("ws://localhost:".concat(CUSTOM_PORT));
        server_1.SubscriptionServer.create({
            schema: schema,
            execute: graphql_1.execute,
            subscribe: graphql_1.subscribe,
        }, customServer);
        var client = new client_1.SubscriptionClient("ws://localhost:".concat(CUSTOM_PORT), {}, MockWebSocket);
        var numTriggers = 0;
        client.request({
        ...
        setTimeout(function () {
            (0, chai_1.expect)(numTriggers).equal(1);
            done();
        }, 200);
    });
});
